# https://github.com/bot66/MNISTDiffusion/blob/main/unet.py
import torch
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        channels_down, channels_up = self._cal_channels(
            base_dim, dim_mults, cond_channels
        )
        ...
        self.encoder_blocks = nn.ModuleList(
            [EncoderBlock(c[0], c[1], time_embedding_dim) for c in channels_down]
        )
        self.decoder_blocks = nn.ModuleList(
            [DecoderBlock(c[0], c[1], time_embedding_dim) for c in channels_up]
        )

        self.mid_block = nn.Sequential(
            ResidualBottleneck(channels_down[-1][1], channels_down[-1][1]),
            ResidualBottleneck(channels_down[-1][1], channels_down[-1][1]),
            ResidualBottleneck(channels_down[-1][1], channels_down[-1][1] // 2),
        )

        self.class_embed = nn.Embedding(n_classes, cond_channels)

        self.final_conv = nn.Conv2d(
            in_channels=channels_up[-1][-1] // 2,
            out_channels=out_channels,
            kernel_size=1,
        )

    def forward(self, x, classes, t=None):
        x = self.init_conv(x)
        if t is not None:
            t = self.time_embedding(t).permute(0, 3, 1, 2)
        encoder_shortcuts = []
        ...
        # shape [B, cond_channels, 1, 1]
        cond = self.class_embed(classes).unsqueeze(-1).unsqueeze(-1)
    ...
    def _cal_channels(self, base_dim, dim_mults, cond_channels):
        dims = [base_dim * x for x in dim_mults]
        dims.insert(0, base_dim)
        channels_down = []
        for i in range(len(dims) - 1):
            channels_down.append((dims[i], dims[i + 1]))  # in_channel, out_channel

        channels_up = [(dims[-1] + cond_channels, dims[-2])]
